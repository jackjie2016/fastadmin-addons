...
if (!is_dir(Env::get("ADDON_PATH"))) {
    @mkdir(Env::get("ADDON_PATH"), 0755, true);
}
...
{
    $results = scandir(Env::get("ADDON_PATH"));
    $list = [];
...
{
    $file = Env::get("ADDON_PATH") . $name . DIRECTORY_SEPARATOR . 'info.ini';
    $addon = get_addon_instance($name);
...
{
    $file = Env::get("ADDON_PATH") . $name . DIRECTORY_SEPARATOR . 'config.php';
